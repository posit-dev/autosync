...
Guidance for AI coding agents working **on** the autosync package.
autosync is a WebSocket sync server for Automerge CRDT documents,
implementing the `automerge-repo` protocol — R serves as a
synchronization hub for Automerge clients in R, JavaScript, Rust, and
other languages.
...
[`auth_config()`](https://posit-dev.github.io/autosync/dev/reference/auth_config.md).
...
[`sync_token()`](https://posit-dev.github.io/autosync/dev/reference/sync_token.md)
...
`dev/RFC-automerge-repo-sync-protocol.md` — consult it before changing
message handling.
